...
class Config extends Search\Config
{
    ...
    // FIXME date map currently unused but broken - see eg Primo to fix
    public function getDateMap($group, $keys)
    {
        $date_arrays = $group->luceneDateToDecade($keys); 
        $map = $date_arrays["display"]; 
        //FIXME need to refactor so keys replaced with new facet_array
        // $facet_array = $date_arrays["decades"]; 
        // FIXME and each entry has is_date set
        // $facet->is_date = true;
       return $map; 
    }
